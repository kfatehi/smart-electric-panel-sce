...
spawn("curl", ["-s", "-XPOST", "http://192.168.1.7:8086/query", "--data-urlencode", "q=CREATE DATABASE studio"]).on('exit', ()=>{
    ...
    fs.createReadStream('/dev/ttyUSB0').on('data', (data)=>{
        ...
        spawn("curl", ["-s", "-XPOST", "http://192.168.1.7:8086/write?db=studio", "--data-binary", dataline]);
    });
});